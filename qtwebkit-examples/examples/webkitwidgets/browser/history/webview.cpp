...
////////////////////////////////////intelligent browser start
    if( tab_index == tabNum)   //{H}hiddentab number
    {
...
cout<< " BrowserApplication::extract_SW == 1\n";
	...
	temp_tabWidget->setCurrentIndex(tabNum);
	}
   }
...
bool WebView::linkClicked(QUrl url) 
{
	...
	for(int i=0; i<tabNum ;i++) //tab change	//{H}hiddentab number
	{
if(BrowserApplication::extract_SW == 0) {	//{H} mod select
		if(!strcmp(temp_tabWidget->get_preurl(i).toString().toUtf8().data(),s_edit_url))	//{HONG} when the request Node is the same with the URL Node
        	{
            	temp_tabWidget->tabBar()->moveTab(i,tabNum);		//{H}hiddentab number
		temp_tabWidget->tabBar()->moveTab(tabNum-1,i); //{H} not need	//{H}hiddentab number
//		temp_tabWidget->tabBar()->tabHide(i, i);	
		temp_tabWidget->tabBar()->tabShow(tabNum);		//{H}hiddentab number
		
		temp_tabWidget->setCurrentIndex(tabNum);	//{H}hiddentab number
