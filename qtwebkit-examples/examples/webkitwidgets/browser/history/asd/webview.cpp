...
QObject::connect(timer, SIGNAL(timeout()), this, SLOT(reload()));
...
//cout << "temp_tabWidget->get_preurl(i).toString().toUtf8().data(),s_edit_url : " << temp_tabWidget->get_preurl(i).toString().toUtf8().data() << ", " << s_edit_url << endl;
...
//{H}
		...
		//if((abc%50) == 0) {
			//write_bufinfo();
		//}
//unsigned long sad = 987654321987654321987654321987654321;
//cout << sad<<endl;
cout << "URL count : " <<abc<<endl;
...
//strcpy(H_url, s_edit_url);
	...
	temp_tabWidget->setCurrentIndex(0);
	load(url);
	//strcpy(H_url, s_edit_url);
	//load(QUrl(s_edit_url));
        //temp_tabWidget->load_hiddenTab(BrowserApplication::gethead(),s_edit_url);
...
void WebView::write_logfile()
{
	...
	ofstream fout;
	cr++;
	fout.open("history/6_ogt1.txt",ios::app);
	if(global_msecs >= 10 && global_msecs <= 50000) {
		if(global_msecs > 0)
			...
			//{	
				//if(strstr(url_data, "Length") == 0) {
					if(strlen(url_data) < 1170) {
						...
						abc++;
						url_2 = edit_url(QUrl(url_data).toString().toUtf8().data());
						load_timer.start();	//{H}
						emit linkClicked(url_2);
						break;
					}
				//}
			//}	
