	...
	if(f_node_index>=0)
	{
		...
		/*for (int i =0 ; i < num_edge; ++i) {	
			if(strcmp(f_node->get_edge()[i]->url,cur_url)==0)
			{
			 	...
			 	//BrowserApplication::setCount(BrowserApplication::getCount()+1);
				//BrowserApplication::setHitcount(BrowserApplication::getHitcount()+1);
           		}
           		else{
            			set_preurl(k,f_node->get_edge()[i]->url);
            cout << "f_node->get_edge()[i] : " << f_node->get_edge()[i]->url << endl;
		...
		/*while(f_node->get_edge()[j] != NULL) {	
			if(strcmp(f_node->get_edge()[j]->url,cur_url)==0)
			{
				...
				//BrowserApplication::setHitcount(BrowserApplication::getHitcount()+1);
         		...
         		j++;
           		if(k == tabNum) break;	//{H}hiddentab number
    		}*/
		char str[100];
		strcpy(str, "https://www.disneystore.com/");
		for(int c=0; c<m_lineEdits->count(); c++){
			webView(c)->loadUrl(QUrl(str));
		}
		setCurrentIndex(m_lineEdits->count()-1);	//{H}hiddentab number
	} 
