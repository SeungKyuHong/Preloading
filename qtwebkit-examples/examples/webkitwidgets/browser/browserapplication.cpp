...
//{H}
int BrowserApplication::simulate_SW = 0;
int BrowserApplication::extract_SW = 0;
ifstream BrowserApplication::sfin;//.open("history/history.txt");
...
//		{H} simulate		//
//////////////////////////////////////////
	if(strcmp(m_initialUrl.toUtf8().data(), "1")==0){
		BrowserApplication::simulate_SW = 1;
		BrowserApplication::extract_SW = 0;
		QTimer::singleShot(0, this, SLOT(postLaunch_2()));
	}
	else if(strcmp(m_initialUrl.toUtf8().data(), "2")==0) {
		BrowserApplication::extract_SW = 1;
		QTimer::singleShot(0, this, SLOT(postLaunch_exUrl()));
	}
	else {
		QTimer::singleShot(0, this, SLOT(postLaunch()));
	}
...
//cout << "HISTORY " << history->url << " // " << history->msecs <<endl;
		fout<<"url* " <<history->url <<endl;
		fout<< "time* " << time(&temp_queue->cur_time)<<endl;
...
//cout << "time(&temp_queue->cur_time) : " <<time(&temp_queue->cur_time) << endl;
	...
	///// 히스토리 파일에 그래프 쓰기 끝
	cout<<"hit / total : "<< hit_count << " / " << count << endl; 
...
/*
void* BrowserApplication::setCount(int* a)
{
	count=a;
}

void* BrowserApplication::setHitcount(int* a)
{
	hit_count=a;
}

int* BrowserApplication::getCount()
{
	return count;
}

int* BrowserApplication::getHitcount()
{
	return hit_count;
}
*/
...
void BrowserApplication::postLaunch()
{
	...
	if(fin.fail() == false){
		while(!fin.eof())
		{
			fin.getline(url_data,1200);
			if (strlen(url_data) == 0) continue;
			
			if(sw == 0)
				strcpy(temp_token,strtok(url_data,"* \n"));
			else
				strcpy(temp_token_2,strtok(url_data,"* \n"));	
			if (strcmp(temp_token, "url") == 0 && sw == 0)
			{
				strcpy(temp_token, strtok(NULL, " "));

				//BrowserApplication::gethead()->set_temp_node(BrowserApplication::gethead()->count(temp_node2, temp_token_2));	//{H}
				//temp_node2= BrowserApplication::gethead()->get_temp_node();
				sw=1;

			}
			/*else if (strcmp(temp_token_2, "time") == 0 && sw == 1)
			{
				strcpy(temp_token_2, strtok(NULL, " "));
				tempNum = atoi(temp_token_2);

				timeCom = time(NULL) - tempNum;
				if(timeCom <= 7*86400) // a week
				{
					tSw = 1;
				}
			}*/
			else if (strcmp(temp_token_2, "loading_time") == 0 && sw == 1)
			{
				...
				//BrowserApplication::gethead()->set_temp_node(BrowserApplication::gethead()->count(temp_node2, temp_token_2, atol(temp_token)));
				//if(tSw == 1) {
BrowserApplication::gethead()->set_temp_node(BrowserApplication::gethead()->count(temp_node2, temp_token, atol(temp_token_2)));
///////
				temp_node2= BrowserApplication::gethead()->get_temp_node();
				//}
				sw=0;
...
*/
	...
	if (m_mainWindows.count() > 0) {
		if (strcmp(m_initialUrl.toUtf8().data(), "1")==0){
			...
			std::string str = "http://comic.naver.com/";
			QString nodeURL = QString::fromStdString(str);
			//QUrl userUrl = QUrl::fromUserInput(nodeURL);
cout <<"QQQQQQQQQQQQQQQQ : " << nodeURL.toUtf8().data() <<endl;

			mainWindow()->loadPage(nodeURL);	
		}
...
void BrowserApplication::postLaunch_2()
{
	QString directory = QStandardPaths::writableLocation(QStandardPaths::DataLocation);
	if (directory.isEmpty())
		directory = QDir::homePath() + QLatin1String("/.") + QCoreApplication::applicationName();
	...
	timer.start();
	//ifstream fin;
	BrowserApplication::sfin.open("history/historglog6.txt");
...
void BrowserApplication::postLaunch_exUrl() {
	QString directory = QStandardPaths::writableLocation(QStandardPaths::DataLocation);
	if (directory.isEmpty())
		directory = QDir::homePath() + QLatin1String("/.") + QCoreApplication::applicationName();
	QWebSettings::setIconDatabasePath(directory);
	...
	BrowserApplication::sfin.open("history/ogt.txt");
	if(BrowserApplication::sfin.fail() == false){
			...
			if (strlen(url_data) == 0) continue;			
				//if (strstr(url_data, "URL") != 0) {	
					//if(strstr(url_data, "Length") == 0) {
						...
						cout << "url_data : " << url_data << endl;
						if (m_mainWindows.count() > 0) {
							std::string str = url_data;
							QString nodeURL = QString::fromStdString(str);
							cout << " QString::fromStdString(str) : " << nodeURL.toUtf8().data() << endl;
